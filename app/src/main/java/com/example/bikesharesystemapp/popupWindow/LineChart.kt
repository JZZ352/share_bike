...
//此页面的作用主要是折现图的内容
class LineChart(val mainActivity: com.example.bikesharesystemapp.MainActivity) {
    ...
    fun showChart() {
        ...
        //关闭按钮
        val imageView = view.findViewById<View>(R.id.chart_close) as ImageView
        imageView.setOnClickListener {
            //都关咯
            CheckViewModel.isVLineChartOpen = false  //电压实时显示
            CheckViewModel.isTLineChartOpen = false   //温度实时显示
            CheckViewModel.isIlluLineChartOpen = false  //光照强度实时显示
...
//      初始化界面
    ...
    private fun generateValues(): PointValue {
        ...
        if (!isChange) { //如果未处于变化状态，值就默认为0
            when {
                CheckViewModel.isVLineChartOpen -> {
                    value = CheckViewModel.getV().toFloat()
                }   //当接收到电压的数据
                CheckViewModel.isTLineChartOpen -> {
                    value = CheckViewModel.getT().toFloat()
                }   //当接受到温度的数据
                CheckViewModel.isIlluLineChartOpen -> {
                    value = CheckViewModel.getIllu().toFloat()
                }   //当接收到光照强度的数据
            }

        }
        isChange = false //赋值后置为false
        //PointValue方法传入的是坐标系的横纵坐标
        return PointValue(numberOfPoints++.toFloat(), value)
    }

    //展示移动中的视图窗口（需要获取实时的横纵坐标）
...
//        }
        if (numberOfPoints > xRegionMax ) { //xRegionMax==6
            pointValueList?.removeAt(0)
        }
...
//看到这来了
        ...
        for (x in 0..999) {
            simpleDateFormat?.format(Date(time + 1000 * x))?.let { timeList?.add(it) }
    ...
    private fun createBroadcast() {
        ...
        filter.addAction("com.example.trafficdetection.currentValue") //动态注册广播
        receiver = object : BroadcastReceiver() {
            override fun onReceive(context: Context?, intent: Intent) {
                if (intent.action == "com.example.trafficdetection.currentValue") {
